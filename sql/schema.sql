...
CREATE TABLE IF NOT EXISTS entrevista (
    id_entrevista INT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    cpf CHAR(11) NOT NULL,
...
CREATE TABLE requisito (
    ...
    CONSTRAINT uq_requisito_vaga_descricao UNIQUE (id_vaga, descricao)
);
...
CREATE TABLE IF NOT EXISTS vaga (
    id_vaga INT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    descricao VARCHAR(255) NOT NULL,
...
ADD CONSTRAINT fk_contato_cpf FOREIGN KEY (cpf) REFERENCES canditato(cpf) ON DELETE CASCADE;
ALTER TABLE entrevista
ADD CONSTRAINT fk_entrevista_candidato FOREIGN KEY (cpf) REFERENCES candidato(cpf);
ALTER TABLE entrevista
ADD CONSTRAINT fk_entrevista_modalidade FOREIGN KEY (id_modalidade) REFERENCES modalidade_entrevista(id_modalidade);
ALTER TABLE entrevista
ADD CONSTRAINT fk_entrevista_status FOREIGN KEY (id_status_entrevista) REFERENCES status_entrevista(id_status);
ALTER TABLE entrevista
ADD CONSTRAINT fk_entrevista_resultado FOREIGN KEY (id_resultado) REFERENCES resultado_entrevista(id_resultado);
ALTER TABLE entrevista
...
ALTER TABLE etapa_do_processo
ADD CONSTRAINT fk_etapas_vaga FOREIGN KEY (id_vaga) REFERENCES vaga(id_vaga);
ALTER TABLE funcionario
ADD CONSTRAINT fk_funcionario_cargo FOREIGN KEY (id_cargo) REFERENCES cargo(id);
ALTER TABLE requisito
ADD CONSTRAINT fk_requisito_vaga FOREIGN KEY (id_vaga) REFERENCES vaga(id_vaga);
ALTER TABLE vaga
ADD CONSTRAINT fk_vaga_cargo FOREIGN KEY (id_cargo) REFERENCES cargo_vaga(id_cargo);
ALTER TABLE vaga
...
COMMENT ON COLUMN entrevista.id_entrevista IS 'Identificador único da entrevista.';
...
COMMENT ON COLUMN modalidade_entrevista.id_modalidade IS 'Identificador único da modalidade de entrevista.';
...
COMMENT ON COLUMN requisito.id_requisito IS 'Identificador único do requisito.';
...
COMMENT ON COLUMN resultado_entrevista.id_resultado IS 'Identificador único do resultado da entrevista.';
...
COMMENT ON COLUMN status_entrevista.id_status IS 'Identificador único do status da entrevista.';
...
COMMENT ON COLUMN vaga.id_vaga IS 'Identificador único da vaga.';
...
WHERE ep.data_etapa BETWEEN '2026-03-01' AND '2026-03-31' -- Q2
SELECT ep.nome AS etapa_nome,
    ep.descricao,
    f.nome AS funcionario,
...
WHERE te.nome LIKE '%entrevista%'
    AND v.salario >= 1000 -- Q3
SELECT f.nome AS funcionario,
    c.nome AS cargo,
    c.nivel
FROM funcionario f
    INNER JOIN cargo c ON f.id_cargo = c.id -- Q4
...
FROM etapa_do_processo ep
    ...
    INNER JOIN cargo c ON f.id_cargo = c.id -- Q5
SELECT c.nome AS candidato,
    cc.tipo_contato,
    cc.valor
FROM candidato c
    LEFT JOIN contato_candidato cc ON c.cpf = cc.cpf -- Q6
SELECT ca.area,
    COUNT(f.id) AS total_funcionarios
FROM funcionario f
    INNER JOIN cargo ca ON f.id_cargo = ca.id
GROUP BY ca.area
HAVING COUNT(f.id) >= 2 -- Q7
SELECT nome,
    cpf
FROM candidato
WHERE cpf IN (
...
WHERE status_entrevista = 'Realizada' -- Q10
SELECT c.nome AS candidato,
